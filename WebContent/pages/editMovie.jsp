...
<body>
<h1>Add New Movie</h1>
	<form action="../editMovie" enctype="multipart/form-data" method="POST">
		<table>

			<tr>
				<td>Movie Title :</td>
				<td><input type="text" name="title" required="required" value="<%=request.getParameter("title")%>"></td>
			</tr>
			<tr>
				<td>Description :</td>
				<td><textarea rows="4" cols="50" name="description" ><%=request.getParameter("description")%></textarea>
			</tr>
			<tr>
				<td>Duration :</td>
				<td><input type="number" name="duration" required="required" min="0"
				max="240" maxlength="3" value="<%=request.getParameter("duration")%>" readonly="readonly"/>
			</tr>
			<tr>
				<td>Poster :</td>
				<td><input type="file" accept=".jpg"  onchange="upload_img(this);" name="poster" /></td>
				<td><img id="img_id" src="../resources/images/<%=request.getParameter("poster") %>.jpg" height="200" width="200" alt="-"/></td>
			</tr>
		</table>
		<input type="hidden" value="<%=request.getParameter("movieId")%>" name="movieId"/>
		<input type="submit" value = "Edit" name="edit"/>
		<% if(request.getParameter("showDelete")==null) {%>
			<input type="submit" value = "Delete Movie" name="delete" style="margin-left: auto"/>
		<%} %>
	</form>
